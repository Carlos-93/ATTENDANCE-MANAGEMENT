...
export default function Header() {
    ...
    return (
        <header className="sticky top-0 w-full flex justify-between md:justify-end items-center py-2 md:py-5 px-5 backdrop-blur-xl bg-black/30 z-10">
            ...
            <div className="flex gap-4 items-center bg-slate-50 p-2 px-6 rounded-lg shadow-lg">
                {user ? (
                    <article className='flex justify-center items-center gap-5'>
                        <Image src={user.imagesrc ? user.imagesrc : '/storage/user/admin.png'}
                            alt={`Imagen ${user.firstname} ${user.lastname}`}
                            width={60} height={60} className="rounded-full border-2 border-teal-500" priority />
